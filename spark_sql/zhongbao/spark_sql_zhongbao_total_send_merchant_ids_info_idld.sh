...
insert overwrite table tubobo_reportdata.zhongbao_total_send_merchant_ids_info partition (day='${start_date}')
select
...
from
(select user_id from ds_tubobo_merchant.bas_tubobo_order_pd where day='${start_date}' and to_date(create_date)='${start_date}' group by user_id)a
join 
(select user_id,address_province,address_city from ds_tubobo_merchant.bas_tubobo_merchant_info_pd where day='${start_date}') b
