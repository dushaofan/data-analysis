...
insert overwrite table tubobo_reportdata.zhongbao_platform_income_details partition (day='${two_start_date}')
select
aaa.create_date,
aaa.province,
aaa.city,
...
aaa.total_gift_account+aaa.total_rider_withdraw_account as outcome_total_amount
from 
(select 
dd.create_date,
dd.province,
...
from 
(select user_id,order_status,to_date(create_date) as create_date from ds_tubobo_merchant.bas_tubobo_order_pd where day='${two_start_date}') a
join 
(select user_id,address_province,address_city from ds_tubobo_merchant.bas_tubobo_merchant_info_pd where day='${two_start_date}' )b on a.user_id=b.user_id 
group by a.create_date,b.address_province,b.address_city) dd 
left join 
(SELECT
a.create_date,
b.address_province as province,
...
FROM
(select account_id,amount,status,type,target,to_date(create_time) as create_date  from ds_tubobo_account.bas_t_account_ops_pd where day='${two_start_date}' )a
join
(select account_id,address_province,address_city from ds_tubobo_merchant.bas_tubobo_merchant_info_pd  where day='${two_start_date}') b 
 on  a.account_id = b.account_id group by a.create_date,b.address_province,b.address_city) aa 
...
from 
(select account_id,amount,sub_type,type,status,target,to_date(create_time) as create_date from ds_tubobo_account.bas_t_account_ops_pd 
where day='${two_start_date}')a
join 
(select account_id,split(pcd_name,' ')[0] as province,split(pcd_name,' ')[1] as city from ds_tubobo_rider.bas_tubobo_rider_info_pd where day='${two_start_date}') b 
 on  a.account_id = b.account_id group by a.create_date,b.province,b.city) bb 
...
SUM(a.amount) as total_commission_account
 from 
(select ops_id,type,amount,to_date(create_time) as create_date from ds_tubobo_account.bas_t_account_record_pd 
where day='${two_start_date}' and type = '999') a
 LEFT JOIN 
(select id,account_id,create_time from ds_tubobo_account.bas_t_account_ops_pd where day='${two_start_date}') b ON a.ops_id = b.id
LEFT JOIN 
(select account_id,address_province,address_city from ds_tubobo_merchant.bas_tubobo_merchant_info_pd where day='${two_start_date}') c 
ON b.account_id = c.account_id
group by a.create_date,c.address_province,c.address_city) cc 
