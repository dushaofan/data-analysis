...
insert overwrite table tubobo_reportdata.zhongbao_merchant_order_heat_entity partition(day='${start_date}')
select 
a.create_date                    ,
...
from ds_tubobo_merchant.bas_tubobo_order_pd where day='${start_date}' and to_date(create_date)='${start_date}' group by user_id,to_date(create_date)) a 
join 
(select user_id,address_province,address_city,address_district,longitude,latitude 
from ds_tubobo_merchant.bas_tubobo_merchant_info_pd where day='${start_date}')b  on a.user_id=b.user_id;"
