...
hive_table=bas_t_account_ops_init
...
#增量的脚本
source /etc/profile
sqoop import --connect ${connect_url} --username ${db_username} --password ${db_password} --query "select id,account_id,order_id,related_op_id,target,amount,amount_from_withdrawable,commission,type,sub_type,status,title,details,remarks,channel,third_system_order_id,client_ip,del_flag,create_time,update_time from t_account_ops where create_time<='${start_time}' or update_time<='${start_time}' and  \$CONDITIONS " -m 1 --hive-table ${hive_database}.${hive_table} --hive-drop-import-delims --target-dir ${target_dir} --hive-overwrite --null-string '\\N' --null-non-string '\\N' --fields-terminated-by "\001" --hive-import;
echo $?

##bas_t_account_record_init
hive_table=bas_t_account_record_init
hive_database=ds_tubobo_account
target_dir=/user/hive/warehouse/tmp.db/${hive_table}
source /etc/profile
hadoop dfs -rmr ${target_dir}
hadoop dfs -rmr /user/hive/warehouse/tmp.db/bas_t_account_record_init
#idld shell
source /etc/profile
sqoop import --connect ${connect_url} --username ${db_username} --password ${db_password} --query "select id,account_id,ops_id,amount,type,sub_type,current_balance,current_withdrawable,current_frozen,current_deposit,remarks,del_flag,create_time,charge_time from t_account_record where charge_time<'${start_time}' or create_time<'${start_time}' and  \$CONDITIONS " -m 5 --hive-table ${hive_database}.${hive_table} --hive-drop-import-delims --target-dir ${target_dir} --hive-overwrite --null-string '\\N' --null-non-string '\\N' --fields-terminated-by "\001" --hive-import;
echo $?

##bas_tubobo_merchant_info_init
...
#增量的脚本
source /etc/profile
sqoop import --connect ${connect_url} --username ${db_username} --password ${db_password} --table ${mysql_table} -m 1 --hive-table ${hive_database}.${hive_table} --hive-drop-import-delims --target-dir ${target_dir} --hive-overwrite --null-string '\\N' --null-non-string '\\N' --fields-terminated-by "\001" --hive-import;
echo $?


# inert bas table 
spark-sql --master ${spark_url} -e "
insert overwrite table ds_tubobo_account.bas_t_account_ops_pd partition (day='${two_start_date}')
select  * from ds_tubobo_account.bas_t_account_ops_init;

insert overwrite table ds_tubobo_account.bas_t_account_record_pd partition (day='${two_start_date}')
select * from ds_tubobo_account.bas_t_account_record_init;

insert overwrite table ds_tubobo_merchant.bas_tubobo_order_detail_pd partition (day='${two_start_date}')
select * from ds_tubobo_merchant.bas_tubobo_order_detail_init;

insert overwrite table ds_tubobo_merchant.bas_tubobo_order_pd partition (day='${two_start_date}')
select * from ds_tubobo_merchant.bas_tubobo_order_init;

insert overwrite table ds_tubobo_merchant.bas_tubobo_merchant_info_pd partition (day='${two_start_date}')
select * from ds_tubobo_merchant.bas_tubobo_merchant_info_idld;

insert overwrite table ds_tubobo_rider.bas_tubobo_rider_info_pd partition (day='${two_start_date}')
