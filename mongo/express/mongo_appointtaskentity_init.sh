...
#mongodump -h ${connect_url} -d ${database} -c ${table} -q "{"createDate":{\$lt:Date(${begin_time_8})}}"
...
source /etc/profile
hive -f /xinguang/mongo/express/mongo_appointtaskentity_hive_init.sql

hive -e "
insert overwrite table ds_tubobo_express.bas_appointtaskentity_init
select 
 a.id                       
,a.taskNo                   
,a.taskType                
...
insert overwrite table ds_tubobo_express.bas_appointtaskentity_pd partition(day='${two_start_date}')
select * from ds_tubobo_express.bas_appointtaskentity_init;" 
