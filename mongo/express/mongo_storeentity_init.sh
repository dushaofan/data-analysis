...
source /etc/profile
#把HDFS临时文件写入到初始化表中
hive -f /xinguang/mongo/express/mongo_storeentity_hive_init.sql
#初始化表中写入到全量表中，每天全量存储
spark-sql --master ${spark_url} -e "
insert overwrite table ds_tubobo_express.bas_storeentity_pd partition (day='${two_start_date}')
select * from ds_tubobo_express.bas_storeentity_init;"
