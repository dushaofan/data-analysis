...
source /etc/profile
#mongodump -h ${connect_url} -d express-admin -c WayBillSEntity -q "{"updateDate":{\$lt:Date(${begin_time_8})}}"
mongodump -h ${connect_url} -u ${username} -p ${password}  -d ${database} -c ${table} -q "{'updateDate':{\$lt:Date(${begin_time_8})}}"
...
source /etc/profile
#把临时路径下的数据load到原表
hive -f /xinguang/mongo/express/mongo_waybillsentity_hive_init.sql
#把原表数据写入到初始化表中
spark-sql --master ${spark_url}  -e "
insert overwrite table ds_tubobo_express.bas_waybillsentity_pd 
select * from ds_tubobo_express.bas_waybillsentity_init ;" 
