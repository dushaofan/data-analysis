...
spark_url=${spark##*=}
table=ServiceProviderEntity
hdfs_path=/apps/hive/warehouse/tubobo_express.mongo/${table}/
#download-mongo-to-loacal-path
source /etc/profile
#mongodump -h ${connect_url} -d express-admin -c ServiceProviderEntity 
...
source /etc/profile
#把临时路径下的数据load到原表
hive -f /xinguang/mongo/express/mongo_serviceproviderentity_hive_idld.sql
#把原表数据写入到初始化表中
hive -e "
insert overwrite table ds_tubobo_express.bas_serviceproviderentity_idld
select 
 a.id           
,a.phone        
,a.name         
...
,a.updateDate   
,a.updateBy
from mongo_tmp_tubobo_express.serviceproviderentity a;"

spark-sql --master ${spark_url} -e "
insert overwrite table  ds_tubobo_express.bas_serviceproviderentity_pd partition (day='${start_date}')
select * from ds_tubobo_express.bas_serviceproviderentity_idld;"
