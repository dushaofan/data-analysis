...
source /etc/profile
#把临时路径下的数据load到原表
hive -f /xinguang/mongo/express/mongo_serviceproviderentity_hive_init.sql
#把原表数据写入到初始化表中
hive -e "
insert overwrite table ds_tubobo_express.bas_serviceproviderentity_init
select 
 a.id           
,a.phone        
,a.name         
...
insert overwrite table ds_tubobo_express.bas_serviceproviderentity_pd partition (day='${two_date_ago}')
select * from ds_tubobo_express.bas_serviceproviderentity_init;" 
