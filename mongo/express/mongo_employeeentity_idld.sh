...
source /etc/profile
#mongodump -h ${connect_url} -d ${database} -c EmployeeEntity 
...
source /etc/profile
#把临时路径的增量数据写入到hive原表中
hive -f /xinguang/mongo/express/mongo_employeeentity_hive_idld.sql
#从原表提取增量数据到增量表中，这一步把map 的id取出 .belongStore[$id] [] 只能在hive中执行，不能在spark执行

spark-sql --master ${spark_url} -e "
insert overwrite table ds_tubobo_express.bas_employeeentity_pd partition(day='${start_date}')
select * from  ds_tubobo_express.bas_employeeentity_idld;"
