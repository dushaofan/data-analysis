...
mysql -h${reoportdata_url} -P3306 -u${mysql_user} -p${mysql_password} $tubobo_reportdata  -e "truncate table tubobo_reportdata.${sqoop_table}"
sqoop export --connect ${mysql_url} --table ${sqoop_table} --export-dir ${export_dir}/day=${start_date}  --username ${mysql_user} -password ${mysql_password} -m 1 --fields-terminated-by '\001' --input-null-string '\\N' --input-null-non-string '\\N' --columns "store_id,store_status,province,city,create_date"
echo $?
...
sqoop export --connect ${mysql_url} --table ${sqoop_table} --export-dir ${export_dir}/day=${start_date} --username ${mysql_user} -password ${mysql_password} -m 5 --fields-terminated-by '\001' --input-null-string '\\N' --input-null-non-string '\\N' --columns "create_date,area_code,store_name,store_id,service_provider_name,area,express_in_num,yesterday_in_num,yesterday_increase,yesterday_reduce"
echo $?
...
sqoop export --connect ${mysql_url} --table ${sqoop_table} --export-dir ${export_dir}/day=${start_date}  --username ${mysql_user} -password ${mysql_password} -m 5 --fields-terminated-by '\001' --input-null-string '\\N' --input-null-non-string '\\N' --columns "area_code,date,province_code,city_code,district_code,area,store_id,store_name,join_time,service_provider_id,service_provider_name,province,city,district,latitude,longitude,latitude_and_longitude,express_in_num,express_sign_num,today_express_sign_num,send_message_num,total_send_message_num,stagnation_num,express_company_num,express_company"
echo $?
